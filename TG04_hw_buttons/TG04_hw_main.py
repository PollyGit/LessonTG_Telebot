...
async def test_button(message: Message):
    await message.answer(f'Пока-пока, {message.from_user.first_name}')
...
async def links(message: Message):
    await message.answer('Продолжим', reply_markup=kb.inline_keyboard_links)


@dp.message(Command('dynamic'))
async def links(message: Message):
    await message.answer('Продолжим', reply_markup=kb.inline_keyboard_dynamic)

@dp.callback_query(F.data == 'catalog')
async def catalog(callback: CallbackQuery):
    await callback.message.edit_text('Новые опции', reply_markup=await kb.inline_dynamic())

#для Inline-кнопок
@dp.callback_query(F.data == 'Опция 1')
async def var1(callback: CallbackQuery):
    await callback.message.answer('Опция 1')

@dp.callback_query(F.data == 'Опция 2')
async def var2(callback: CallbackQuery):
